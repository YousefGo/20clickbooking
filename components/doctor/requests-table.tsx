...
import { doctorConfirmAppointment, doctorRejectAppointment } from "@/lib/actions/doctor-appointments";
import type { Dictionary } from "@/lib/i18n/get-dictionary";
import type { Locale } from "@/lib/i18n/config";

export interface DoctorPendingRequestItem {
  id: string;
  patientName: string;
...
export function DoctorRequestsTable({
  locale,
  dict,
  timezone,
  items,
  ...
  dict: Dictionary;
  timezone: string;
  items: DoctorPendingRequestItem[];
}) {
...
function RequestCard({
  ...
  function handleConfirm() {
    startTransition(async () => {
      const result = await doctorConfirmAppointment(item.id);
      if (!result.ok) {
  ...
  function handleReject(reason: string, notifyPatient: boolean) {
    startTransition(async () => {
      const result = await doctorRejectAppointment(item.id, { reason, notifyPatient });
      if (!result.ok) {
        toast.error(dict.common.error);
        return;
      }
